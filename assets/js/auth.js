...
export const login = async (email, password) => {
    try {
        ...
        if (!userData) {
            console.warn("Profilo Firestore mancante per l'utente loggato. Tentativo di ripristino automatico...");
            // Se il documento manca, lo ricreiamo con dati base (auto-ripristino per sito tra amici)
            try {
                ...
                console.error("Errore durante il ripristino automatico del profilo:", restoreError);
                await signOut(auth);
                return { user: null, error: "Errore durante il ripristino del profilo. Contatta l'amministratore." };
            }
        }
        ...
        else if (error.code === 'auth/user-disabled') message = "Questo account è stato disabilitato dall'amministratore.";
...
export const register = async (email, password, displayName) => {
    try {
        ...
        // Firebase effettua il login automatico al momento della registrazione.
        // Restituiamo l'utente per confermare il successo.
        return { user, error: null };
    } catch (error) {
        console.error("Registration Error:", error);
        let message = "Errore durante la registrazione.";
        
        // Messaggi di errore localizzati e corretti
        switch (error.code) {
            ...
            case 'auth/operation-not-allowed':
                message = "La registrazione con email/password non è abilitata.";
                break;
            case 'auth/weak-password':
                message = "La password è troppo debole. Usa almeno 6 caratteri.";
                break;
            case 'permission-denied':
                message = "Errore di database: permessi insufficienti.";
                break;
            default:
